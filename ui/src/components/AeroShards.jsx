...
export default function AeroShards({
  ...
  // Computed via useMemo (not read back from settingsRef) so the effect below
  // can depend on it without accessing a ref during render.
  ...
  // Written in a layout effect (runs every render, synchronously before
  // paint) rather than during render, so the many effects/animation-loop
  // callbacks below that read settingsRef.current always see this render's
  // values without the ref write itself violating render purity.
  useLayoutEffect(() => {
    settingsRef.current = {
