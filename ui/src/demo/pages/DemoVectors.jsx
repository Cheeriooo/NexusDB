...
function fakeLatency(min = 150, max = 500) {
    return new Promise((resolve) => setTimeout(resolve, min + Math.random() * (max - min)));
}

export default function DemoVectors({ addToast, collections, setCollections }) {
    ...
    const bumpCount = (n) => setCollections((prev) => prev.map((c) => (c.name === selected ? { ...c, count: c.count + n } : c)));
    ...
    const handleJsonUpsert = async (e) => {
        ...
        try {
            ...
            await fakeLatency();
            bumpCount(parsed.length);
            setResult({ count: parsed.length, ids: parsed.map((v) => v.id) });
            addToast(`Inserted ${parsed.length} vectors`, 'success');
        } catch (err) {
    ...
    const handleEmbedUpsert = async (e) => {
        ...
        setLoading(true);
        try {
            await fakeLatency(300, 800);
            const ids = lines.map((_, i) => (idPrefix ? `${idPrefix}${i + 1}` : `vec_${i + 1}`));
            bumpCount(lines.length);
            setResult({ count: lines.length, ids, model: embedModel });
            addToast(`Embedded & inserted ${lines.length} vectors`, 'success');
        } catch (err) {
            addToast(err.message, 'error');
            setResult(null);
        } finally {
    ...
    return (
        <div className="vectors-page">
            ...
            <div className="card">
                <div className="card-body">
                    <div className="mode-tabs">
                        <button type="button" className={`mode-tab ${mode === 'json' ? 'active' : ''}`} onClick={() => { setMode('json'); setResult(null); }}>
                            JSON / Raw vectors
                        </button>
                        <button type="button" className={`mode-tab ${mode === 'embed' ? 'active' : ''}`} onClick={() => { setMode('embed'); setResult(null); }}>
                            ✦ Text Embed
                        </button>
                    </div>
                        ...
                        <form onSubmit={handleEmbedUpsert}>
                            <div className="form-group" style={{ marginTop: '16px' }}>
                                ...
                                <select className="form-select" value={embedModel} onChange={(e) => setEmbedModel(e.target.value)}>
                                    {EMBED_MODELS.map((m) => (
                                        <option key={m.id} value={m.id}>{m.label} — {m.dim}D — {m.desc}</option>
                                    ))}
                                </select>
                            </div>
                            ...
                            <div className="form-group">
                                ...
                                <input type="text" className="form-input" value={idPrefix} onChange={(e) => setIdPrefix(e.target.value)} placeholder="doc_" disabled={dimMismatch} />
                            </div>

                            <div className="form-actions">
                                <button type="submit" className="btn btn-primary" disabled={loading || !selected || dimMismatch || !textInput.trim()}>
            ...
            <AnimatePresence>
                {result && (
                    <motion.div className="card result-card" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}>
